...
| **T** ✓ | **On-demand context retrieval — all 4 chats wired**. The 4 chat graphs (refinement / blueprint / inspector / planner) now run a tool-calling pre-step via `runAgentLoop` before producing structured output, mirroring the existing pattern in architect/controller. Each chat gates RAG tools on corpus availability (`countEpicEmbeddings` / `countTicketEmbeddings`) so first-run orgs skip the loop entirely — graceful degradation, no LM Studio fallback needed (Gemma simply doesn't call tools, so `augmented == initial`). **Per-chat tool sets**: Refinement gains `get_ticket_details` (NEW — `tools/getOtherTicketProposal.ts`, closure-captures the in-session backlog so the LLM only passes a `prop-xxxxxxxx` id; rejects self-reference and unknown ids with a `{ error, knownIds }` payload) plus `find_similar_tickets` + `find_similar_epics`. Blueprint gains `find_similar_epics` (the backlog itself is already inline in the prompt, so siblings need no tool). Inspector gains `find_similar_epics` + `find_similar_tickets` (Living-Memory persona — cross-Epic search is its core value-add). Planner gains the same two RAG tools, focused on effort-estimation anchoring. **Prompt updates**: each chat prompt now has an `==== ON-DEMAND CONTEXT (TOOLS) ====` section enumerating available tools, when to call them, and explicit "skip-this-tool" cases (don't search for cosmetic edits, paraphrase results, never dump JSON). Refinement also adds a `SIBLINGS` section showing `#N Title [label] \| id=prop-xxx` so the AI can translate between PO-facing position numbers and tool-facing ids. **Registry**: `OrionPhase` enum extended with chat scopes `blueprintChat \| refinementChat \| plannerChat \| inspectorChat`; registry test updated. **Resolvers + signatures**: `runBlueprintChat` / `runRefinementChat` / `runPlannerChat` now accept `ctx?: { orgId? }`; resolvers pass `{ orgId: ctx.orgId }`. `inspectorServer` threads `orgId` through to `runInspectorTurn`. Files: `tools/registry.ts`, `tools/getOtherTicketProposal.ts` + test, `realAi/{refinement,blueprint,inspector,planner}ChatGraph.ts`, `realAi/inspectorServer.ts`, `graphql/resolvers.ts`. | Type-check + 120 vitest pass; lint clean; manual smoke pending |

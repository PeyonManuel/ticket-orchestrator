...
| Backend        | All 7 actors wired to real Gemini via **LangChain** (Slice J). Server-only graph files in `src/infrastructure/orchestrator/realAi/`. GraphQL mutations + Apollo client adapter. `NEXT_PUBLIC_MOCK_AI=1` manual fallback for fast CI.                                  | Tool-augmented agents (RAG, AC linter, semantic point estimator) — Slices K+                                                                                                                            |
| E2E            | Playwright bootstrapped; two specs (orchestrator-commit, inspector) cover approve happy paths (Slice G)                                                                                                                                                               | Over-capacity branch + back-to-refine branch (deferred to Slice H+I)                                                                                                                                    |
...
| **K**     | **Tool infrastructure (FIRST tool slice)**. New `src/infrastructure/orchestrator/tools/{index,registry}.ts` with `OrionTool` type, `toolsForPhase(phase)` selector, `registerTool` API, empty per-phase buckets. New `bindOrionTools(llm, tools)` helper in `llm.ts`. `controllerGraph.ts` calls `toolsForPhase("phase3")` with a loud guard so Slice M can't register tools without wiring the agent loop. `scripts/smokeTools.ts` proves a no-op echo tool gets invoked end-to-end. No user-facing change.                                                                                                                                                                                                                             | Type-check passes; existing smokes pass; new smoke proves the model calls a registered tool                  |

...
| **H** | Phase back-navigation: back-transitions in the XState machine, styled "going back" confirmation modal, artifact-clear + transcript-append pattern, remove REGENERATE_PLAN / Revise Plan button | Manual test all back paths |
| **I** | Proposal label simplification: `ProposalLabel` → `"ux" \| "developer" \| "po" \| "qa"`, UI chip updates, on-read backcompat for old draft data | Type-check; existing drafts still load |
| **J** *(later)* | Swap mock actors for real LangGraph adapters; same signatures | Integration tests |

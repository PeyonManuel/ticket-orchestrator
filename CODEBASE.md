...
- **UX polish + session persistence (Slice S)** — `shared/ProseTurn.tsx` extracted (paragraph-split prose, no bubbles) and applied uniformly across all 5 phase chats. All 5 chat panels auto-scroll to the tail on mount (returning to a paused draft shows the most recent turn). `useOrchestrator` restores the last visited phase on resume via `RESUME_PHASE` instead of always landing on Phase 1. Blueprint + refinement prompts instruct the AI to reference tickets by `#N Title` in narrative while using `prop-xxxxxxxx` ids in mutations.
- **On-demand context retrieval (Slice T)** — all 4 chat graphs (refinement / blueprint / inspector / planner) run a tool-calling pre-step via `runAgentLoop` before structured output. Per-chat tool sets: refinement gains `get_ticket_details` (NEW — `tools/getOtherTicketProposal.ts`, closure-captures sibling backlog) plus `find_similar_tickets` + `find_similar_epics`; blueprint gains `find_similar_epics`; inspector gains both RAG tools; planner gains both RAG tools. All RAG tools gated on corpus availability — first-run orgs skip the loop. `OrionPhase` enum extended with chat scopes. Each prompt has an `==== ON-DEMAND CONTEXT (TOOLS) ====` section enumerating when to call and when to skip.
- **Drag-and-drop conflict UI** — `dispatchTicketPatch` now auto-opens `TicketModal` on `ConflictError` (in addition to setting `conflictError` state), so out-of-modal patch paths (drag-and-drop, programmatic moves) surface the existing Overwrite/Discard banner instead of failing silently.
- **Phase 4 per-discipline visibility** — each `SprintLane` now renders a compact per-discipline used/cap row (`Dev 12/16 · UX 5/10 · QA 3/5 · PO 0/3`) beneath the total-SP capacity bar, color-graded green/amber/rose at the 80% / 100% thresholds. Mirrors the planner's discipline math via the now-exported `ticketDiscipline` helper from `slicingPolicy`, so the PO sees the same partition the slicing policy uses. `CapacityPanel`, `OverflowBanner`, and the "Approve & commit" CTA were already in place — this closes the last visualization gap from Slice C.

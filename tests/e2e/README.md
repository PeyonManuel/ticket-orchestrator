...
- **Phase 4 "over-capacity" branch** is not yet covered — needs a dedicated
  seed that produces a backlog overflowing the planner's buffer rule.
- **Phase 4 back-to-refine** flow lands in Slice H+I (back-navigation modal).
  Add a spec there.
- **Auth setup selectors** target Clerk's default sign-in form; if the
  hosted UI has been heavily themed, `auth.setup.ts` may need adjustment.
